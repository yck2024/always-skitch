...
- Implementation has two halves, both required:
  - The **dismissal guard** at the `mouse:up` text-creation branch (`handleCanvasClick` in Skitch, the click-place branch of `handleMouseUp` in Freeform). Before constructing a new Textbox, the handler iterates `canvas.getObjects()` and finds any object where `data.kind === 'text'` AND `isEditing === true`. If found, the handler does a bounding-box hit-test (`editingText.containsPoint(pointer)`): clicks inside the text body fall through so IText's hidden textarea can reposition the cursor; clicks outside trigger `exitEditing()` and consume the event. This catches the click-outside path.
...
- The guard **must iterate** `canvas.getObjects()` rather than read `canvas.getActiveObject()`. Both `addFinalObject` implementations set `evented = false` (and `selectable = false`) on any annotation created while a sticky drawing tool is active — including a freshly-created Text — so the next click starts a new annotation instead of grabbing the previous one. Fabric's `IText.enterEditing()` (around line 16859 in `index.mjs`) sets `isEditing = true` and focuses the hidden textarea but does NOT call `setActiveObject(this)`. Net result: while a text is being edited, it is NOT the canvas's active object. `getActiveObject()` returns `null` or some unrelated object the user happened to click; the guard would fall through to creation and spawn the redundant text box. The iteration is bounded — Fabric's `textEditingManager.exitTextEditing()` runs inside `enterEditingImpl`, so at most one text is in editing state per canvas. Likewise the in-text click check uses `containsPoint(pointer)` (a bounding-box hit-test against the scene-space pointer) rather than `event.target === editingText`, because Fabric does not report non-evented objects as mouse-event targets.
